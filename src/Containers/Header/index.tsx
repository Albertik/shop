...
// images and/or icons
import Logo from '../../Assets/Images/logo_julia.webp';
	...
	const renderMainHeader = (): JSX.Element => {
		if (isSearching) {
			...
			return (
				<React.Fragment>
					{isProduct ? (
						<ButtonSmall to={history} from={window.location.pathname} source={ArrowLeft} isCTA={true} dispatch={dispatch} />
					) : (
					...
					<nav className='Header__buttons-container'>
						{window.location.pathname === '/' && <ButtonSmall source={Search} onclick={handleSearch} />}

						<ButtonSmall to='/menu' from={window.location.pathname} dependencies={shoppingCart.length} source={Menu} dispatch={dispatch} />
					</nav>
					...
					<nav className='Header__desktop'>
						...
						{window.location.pathname !== '/shopping-cart' && (
							<button className='Header__cart' onClick={() => navigate('shopping-cart')}>
								<span>Cart</span>
								<img src={Cart} alt='' />
								{shoppingCart.length > 0 && <span className='Header__cart--notification'></span>}
							</button>
						)}
					</nav>
				</React.Fragment>
